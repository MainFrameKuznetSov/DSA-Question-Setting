...
class A
{
	public static void main(String[] args) 
	{
		...
		while(t-->0)
		{
		    ...
		    for(int i=0;i<n;++i)
		        arr[i]=ms.nextInt();
		    HashMap<Integer,Integer>fapp=new HashMap<>();
		    HashMap<Integer,Integer>lapp=new HashMap<>();
		    for(int i=0;i<n;++i)
		    {
		        lapp.put(arr[i],i);
		        if(!fapp.containsKey(arr[i]))
		            fapp.put(arr[i],i);
		    }
		    for(int i=1;i<=k;++i)
		    {
		        int a=ms.nextInt();
		        int b=ms.nextInt();
		        if(!fapp.containsKey(a) || !lapp.containsKey(b))
		        {
		            pw.println("NO");
		            continue;
		        }
		        int l=fapp.get(a),r=lapp.get(b);
		        //pw.println(l+" "+r);
